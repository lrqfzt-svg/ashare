...
TEMPLATE_CSS = """  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .money-plus { color: #f85149; }
  .money-minus { color: #3fb950; }
...
SAMPLE_TEMPLATE_DATA = {
  ...
  "opcheck_cols": [
    {"title":"✅ 下周一（08-24）· 进攻方向","cls":"#3fb950","items":[
      "🚀 空间板 汉森制药（3板）作为情绪锚：高开不追、换手回封再上车；炸板翻绿则全场连板降仓。",
      "🎯 主线 通信设备（主力净流入+276.4亿）：只做前排换手核心，分歧急杀不破5日线可低吸，后排不追。",
      "🔥 连板梯队完整（3板1只、2板5只）：优先卡位3进4弱转强，其次2进3换手板。",
      "⏱️ 出手节奏：早盘不急着追，等10:00后分歧转一致，午后回封比早盘秒板更安全。",
    ]},
    {"title":"🛡️ 防守纪律","cls":"#f85149","items":[
      "🚫 医药生物板块（主力净流出-58.3亿）：坚决不碰、持仓择机减。",
      "💰 量能健康（成交1.89万亿）：仓位可放5–7成，分仓滚动、不重仓单票。",
      "🛑 铁律：单日亏损≥5%强制降仓；不补仓摊平、不借钱、不All-in单一题材。",
    ]},
    {"title":"🔭 盘中盯盘信号","cls":"#58a6ff","items":[
      "🔭 量能：竞价成交较前日放量则顺势，持续缩量则收手等尾盘。",
      "👑 空间板 汉森制药：封单≥3亿且全天未漏则情绪安全；炸板翻绿=连板退潮，立即减仓。",
      "🌊 主线资金：盯 通信设备 开盘30分钟主力净流入是否延续，转净流出则板块分歧、降预期。",
      "🎯 买点确认：只做「板块资金净流入+个股换手充分+分时回封」三者共振，缺一不行动。",
    ]},
  ],
...
{chr(10).join(groups)}
    ...
    L.append(f"""  <div class="section">
    <div class="section-title"><span class="icon">💥</span> 炸板深度分析（炸板{d.get("zha_count",0)}只 · 炸板率{d.get("zbr_s","—")}）</div>
    <div class="two-col">
    ...
    <div class="two-col">
      <div class="col"><h4 style="color:#f85149;">✅ 今日主力净流入 Top</h4>{inrows}</div>
      <div class="col"><h4 style="color:#3fb950;">🚨 今日主力净流出 Top</h4>{outrows}</div>
    </div>
  </div>""")
...
{chr(10).join(trows)}
    ...
    # 游资研判（龙虎榜视角）—— A 规则化 + B 文字研判
    yz = d.get("youzi") or {}
    if yz.get("has"):
        hot_rows = "".join(
            f'<tr><td><b>{x["name"]}</b></td>'
            f'<td style="text-align:right;color:#f85149;">{x["hot_yi"]:+.2f}亿</td>'
            f'<td style="text-align:right;color:#d29922;">{(x["change"] if isinstance(x.get("change"), str) else "—")}</td>'
            f'<td style="color:#8b949e;">{"、".join(x["concepts"][:2]) or "—"}</td></tr>'
            for x in yz.get("hot_top", [])) or '<tr><td colspan="4" style="color:#8b949e;">无游资净买标的</td></tr>'
        div_rows = "".join(
            f'<tr><td><b>{x["name"]}</b></td>'
            f'<td style="text-align:right;color:#58a6ff;">{x["org_yi"]:+.2f}亿</td>'
            f'<td style="text-align:right;color:#f85149;">{x["hot_yi"]:+.2f}亿</td>'
            f'<td style="color:#8b949e;">{x["trend"]}</td></tr>'
            for x in yz.get("diverge", [])[:5]) or '<tr><td colspan="4" style="color:#8b949e;">无显著分歧</td></tr>'
        theme_s = "、".join(f"{t}({v:.1f}亿)" for t, v in yz.get("themes", [])[:6]) or "无显著聚焦"
        L.append(f"""  <div class="section">
    <div class="section-title"><span class="icon">🐉</span> {yz.get('title','游资视角')}</div>
    <div class="two-col">
      <div class="col">
        <h4 style="color:#f85149;">💰 游资净买 Top（hot_yi）</h4>
        <table style="width:100%;font-size:12px;border-collapse:collapse;">
          <tr style="color:#8b949e;"><th style="text-align:left;">个股</th><th style="text-align:right;">游资净额</th><th style="text-align:right;">涨幅</th><th style="text-align:left;">题材</th></tr>
          {hot_rows}
        </table>
      </div>
      <div class="col">
        <h4 style="color:#58a6ff;">🔀 机构 vs 游资分歧</h4>
        <table style="width:100%;font-size:12px;border-collapse:collapse;">
          <tr style="color:#8b949e;"><th style="text-align:left;">个股</th><th style="text-align:right;">机构</th><th style="text-align:right;">游资</th><th style="text-align:left;">结构</th></tr>
          {div_rows}
        </table>
      </div>
    </div>
    <div style="background:#21262d;border-radius:8px;padding:12px 14px;margin:12px 0;font-size:12px;color:#8b949e;">
      📌 <b style="color:#d29922;">游资主攻题材</b>（按 hot_yi 聚合）：{theme_s}
    </div>
    <div style="background:#1c2128;border:1px solid #2d333b;border-radius:8px;padding:14px;font-size:13px;line-height:1.9;color:#c9d1d9;">
      {yz.get('text','')}
    </div>
  </div>""")

    # 操作建议
    L.append(_strategy_section("操作建议", d["opcheck_cols"]))
      ...
      try{{
        ...
        c2.setOption({{ grid:{{left:8,right:36,top:10,bottom:10,containLabel:true}},
          ...
          series:[{{type:'bar',barMaxWidth:18,
            data:fdata.map(function(r){{return {{value:r.val,itemStyle:{{color:r.out?'#3fb950':'#f85149'}}}};}}),
